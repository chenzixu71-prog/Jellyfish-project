...
export default function CreatePage() {
  ...
  async function loadKnowledgeBases() {
    ...
    try { setKnowledgeBases(await getKnowledgeBases()) } catch (error) {
      setKnowledgeBases([])
      setListError(error instanceof Error ? error.message : '知识库加载失败')
    ...
    <View className='create-page'>
      ...
      <View className='content-shell'>
        ...
        <View className='kb-card'>
          ...
          {!listLoading && listError && <View className='list-state'><Text className='empty-text state-error'>{listError}</Text><Button className='retry-button' onClick={loadKnowledgeBases}>重新加载</Button></View>}
          {!listLoading && !listError && knowledgeBases.length === 0 && <Text className='empty-text'>还没有知识库。输入一个主题，让水母帮你整理。</Text>}
          {!listLoading && !listError && knowledgeBases.map((item) => (
