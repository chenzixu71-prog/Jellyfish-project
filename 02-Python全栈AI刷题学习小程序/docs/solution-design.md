...
| 用途 | 默认模型 | 说明 |
| --- | --- | --- |
| 生成闯关题 | `deepseek-v4-flash` | 默认使用，生成 3 道单选题、1 道多选题、1 道判断题 |
...
后端调用 DeepSeek 生成题目时，使用以下系统 Prompt 模板：
...
| type | 含义 | answer 规则 |
| --- | --- | --- |
| `single` | 单选题 | `answer` 只能有 1 个选项 key |
| `multiple` | 多选题 | `answer` 可以有多个选项 key |
| `judge` | 判断题 | `answer` 使用 `["true"]` 或 `["false"]` |

后端必须校验：

- `questions` 是否存在。
- 题目总数是否为 5。
- 题型数量是否为 3 道 `single`、1 道 `multiple`、1 道 `judge`。
- 单选题和多选题是否有 4 个选项。
- 判断题是否有清晰的正确/错误选项或可直接映射为 `true/false`。
- `answer` 是否存在，且每个答案都在合法选项范围内。
- `explanation` 是否为空。
- `knowledge_point` 和 `difficulty` 是否存在。
...
用户完成第 5 题后，不直接跳转报告页。前端先进入庆祝状态：

```text
quiz_completed -> celebrating -> report_waiting_click
```
...
- 中央出现“恭喜完成本轮闯关！”。
- 红、黄、蓝彩带从顶部快速飘洒。
- 彩带在 800-1200ms 内渐隐消失。
- “查看学习报告”按钮在彩带渐隐时出现。

用户点击“查看学习报告”后：

```text
report_waiting_click -> report_loading -> report_ready
```
